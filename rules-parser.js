...
function parseRulesText(text) {
  ...
  for (const block of blocks) {
    try {
      ...
      console.warn('[DomainRedirector] Rule parse error:', e.message, '\nBlock:', block);
    }
  }
  return rules;
}
...
function parseMatchPattern(str) {
  ...
  if (afterList) {
    // Strip outer parens
    const inner = afterList.replace(/^\(/, '').replace(/\)$/, '').trim();
...
function applyRules(urlStr, rules) {
  ...
  for (const rule of rules) {
    ...
    if (match) {
      const item = rule.replaceItems[Math.floor(Math.random() * rule.replaceItems.length)];
...
function tryMatchRule(hostname, rule) {
  ...
  for (const item of matchItems) {
    ...
    const hostPart = item.type === 'literal'
...
function testRules(urlStr, rules) {
  let url;
  try { url = new URL(urlStr); } catch { return { error: 'Invalid URL' }; }
