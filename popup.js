...
chrome.storage.local.get({ dailyRedirects: {}, lastDay: '' }, data => {
  const today = new Date().toISOString().slice(0, 10);
  const count = data.lastDay === today ? (data.dailyRedirects[today] || 0) : 0;
  matchCountEl.textContent = count;
});

// Toggle
toggle.addEventListener('change', () => {
  chrome.runtime.sendMessage({ type: 'setEnabled', value: toggle.checked }, () => {
...
document.getElementById('testUrl').addEventListener('click', () => {
  chrome.tabs.query({ active: true, currentWindow: true }, tabs => {
    if (!tabs[0]) return;
    chrome.tabs.create({
      url: chrome.runtime.getURL('editor.html') + '?testUrl=' + encodeURIComponent(tabs[0].url)
    });
    window.close();
  });
});
